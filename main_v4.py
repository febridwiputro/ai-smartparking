...
from src.controllers.detection_controller_v4 import DetectionControllerV4
...
class Wrapper:
    ...
    def main(self):
        ...
        for i in range(len(caps)):
            ...
            plat_detects[i] = DetectionControllerV4(arduino_text)
            ...
            while True:
                for i in range(len(caps)):
                    ...
                    if plat_detects[i] is not None:
                        ...
                        results = plat_detects[i].get_plate_results()

                        if results:
                            # print("results ========", results)
                            self.plate_result_queue.put(results)

                    # # Process the output from the plate detection queue
                    # if not self.plate_result_queue.empty():
                    #     last_result = self.plate_result_queue.get()
