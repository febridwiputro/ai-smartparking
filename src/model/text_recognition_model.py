...
class TextRecognition:
    ...
    def detect_and_recognize(self, image):
        ...
        bounding_boxes = self.ocr_net.detect_text(image)
        converted_bboxes = []
        ...
        texts = self.ocr_net.read_text(image)
    ...
    def detect_text(self, image):
        t0 = time.time()
        detections = self.reader.detect(image)
        t1 = time.time() - t0
        # logging.info(f"Detection time: {t1:.3f}s")
        return detections[0] if detections else []

    def read_text(self, image):
        t0 = time.time()
        results = self.reader.readtext(image, text_threshold=0.7, low_text=0.4, decoder='greedy')
        t1 = time.time() - t0
        logging.info(f"Reading time: {t1:.3f}s")
        return results if results else []

    def process_image(self, image):
        processed_image = self.image_processing(image)
        detections = self.detect_text(processed_image)
        ...
        for bbox in detections:
            x_min, x_max, y_min, y_max = bbox[:4]
            cropped_image = processed_image[y_min:y_max, x_min:x_max]
            texts.append(cropped_image)

        results = [self.read_text(text) for text in texts]
        
        final_text = self.combine_and_filter_text(results)
