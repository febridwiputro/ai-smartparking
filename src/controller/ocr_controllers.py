...
class OCRController:
    ...
    def car_direct(self, frame, arduino_idx, cam_idx='a') -> tuple:
        floor_position, cam_position = self.check_floor(cam_idx=cam_idx)
        ...
        # Proses lain seperti sebelumnya
        if car.shape[0] == 0 or car.shape[1] == 0:
            return frame, None, total_slot, floor_position, cam_position, poly_points, None
        ...
        plate_no = text if text else ""

        # Kembalikan informasi yang dibutuhkan
        return frame, plate_no, total_slot, floor_position, cam_position, poly_points, bbox
    ...
    #     show_text(f"Total Car: {total_slot}", frame, 5, 150)
    ...
    #     # start, middle, end, end_2 = self.check_centroid_location(results, poly_points, inverse=self.car_direction)        
    #     # return [car, start, middle, end, end_2]

    def processing_car_counter(self, list_data, car_direction=None):
        # Jika list_data hanya memiliki dua elemen
        if len(list_data) == 2:
            plate_no, total_slot = list_data
            # Proses sesuai dengan dua nilai
            print(f"Processing car: Plate No: {plate_no}, Total Slot: {total_slot}")
            return

        # Jika list_data memiliki tiga elemen (logika asli)
    ...
    def processing_ocr(self, arduino_idx, cam_idx, frame, list_data, car_direction=None):
        if len(list_data) == 2:
            # Jika hanya ada dua elemen dalam list_data
            plate_no, total_slot = list_data
            # Tambahkan logika untuk mengatasi dua elemen
            print(f"Processing OCR for Plate No: {plate_no}, Total Slot: {total_slot}")
            return "", frame, np.array([])

        # Jika ada tiga elemen (logika asli)
    ...
    def send_plate_data(self, floor_id, plate_no, cam_position, generate_uuid):
        ...
        self.db_mysn.send_data_to_mysn(params, send_date)
    ...
    def processing_logic_car(self, arduino_idx, cam_idx, status_car):
        ...
        # if len(self.container_text) == 0 and len(self.real_container_text) == 0:
            self.passed = 0
            return
        ...
        self.vehicle_total = matrix_update.get_total()
        self.total_slot = max_slot - self.vehicle_total
        self.plate_no = plate_no

        print(f"PLAT_NO : {plate_no}, TOTAL_CAR : {self.vehicle_total}, STATUS : {'TAMBAH' if not status_car else 'KURANG'}, FLOOR : {floor_position}, CAMERA : {cam_position}, TOTAL_FRAME: {len(self.container_plate_no)}")
        ...
        # self.send_plate_data(floor_id=floor_id, plate_no=plate_no, cam_position=cam_position, generate_uuid=uuid.uuid4())