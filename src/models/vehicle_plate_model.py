...
logger = Logger("VehicleDetector", is_save=True)

class VehicleDetector:
    def __init__(self, model_path, base_dir, is_vehicle_model):
    ...
    def save_dimensions_to_excel(self, height, width, floor_id, cam_id):
        ...
        # Generate the file path and ensure the directory exists
        current_date = datetime.now().strftime("%Y_%m_%d")
        timestamp = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        dataset_dir = os.path.join(self.DATASET_DIR, "log_plate_size")
        ...
        file_path = os.path.join(dataset_dir, f"{current_date}.xlsx")

        # Create new data to append
        new_data = pd.DataFrame(
            [[height, width, floor_id, cam_id, timestamp]],
            columns=['height', 'width', 'floor_id', 'cam_id', 'created_date']
        )

        # Write to Excel (append if file exists, else create new)
        with pd.ExcelWriter(file_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            startrow = writer.sheets['Sheet1'].max_row if os.path.isfile(file_path) else 0
            new_data.to_excel(writer, index=False, header=not os.path.isfile(file_path), startrow=startrow)
        ...
        if is_centroid_inside:
            ...
            if self.frame_count_per_object[self.object_id] <= self.max_num_frame:
                for obj in plate_data:
                    ...
                    bg_color = check_background(gray_plate, self.BASE_DIR, False)
        ...
        if is_centroid_inside and not self.is_vehicle_model:
            ...
            if self.frame_count_per_object[self.object_id] < self.max_num_frame:
                for plate_frame in plate_frames:
                    ...
                    bg_color = check_background(gray_plate, self.BASE_DIR, False) if gray_plate is not None else None
