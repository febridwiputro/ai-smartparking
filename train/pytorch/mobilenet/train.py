...
class Train:
    def __init__(self, dataset_path, img_size=(80, 80), batch_size=64, epochs=30, lr=1e-4, output_dir=None, patience=10, model_version="mobilenet_v2", device='cuda'):
    ...
    def create_model(self):
        if self.model_version == "mobilenet_v2":
            self.model = models.mobilenet_v2(pretrained=True)
        elif self.model_version == "mobilenetv3_small":
            self.model = models.mobilenet_v3_small(pretrained=True)
        elif self.model_version == "mobilenetv3_large":
    ...
    def create_mobilenet_v2(self):
        ...
        self.model.classifier[1] = nn.Linear(self.model.last_channel, len(self.lb.classes_))  # Adjust final layer
        self.model = self.model.to(self.device)
        print(f"Using MobileNet V2 with {len(self.lb.classes_)} output classes")

    def create_mobilenet_v3_small(self):
        self.model = models.mobilenet_v3_small(pretrained=True)
        self.model.classifier[3] = nn.Linear(self.model.classifier[3].in_features, len(self.lb.classes_))
        self.model = self.model.to(self.device)
        print(f"Using MobileNet V3 Small with {len(self.lb.classes_)} output classes")
...
if __name__ == "__main__":
    ...
    if args.version == 'mobilenet_v2':
        train.create_model = lambda: train.create_mobilenet_v2()
    elif args.version == 'mobilenet_v3_small':
        train.create_model = lambda: train.create_mobilenet_v3_small()
    elif args.version == 'mobilenet_v3_large':
        train.create_model = lambda: train.create_mobilenet_v3_large()

    # Run the appropriate mode (train, eval, test)
    if args.mode == 'train':
        train.load_dataset()
        train.create_model()
        train.train_model()
